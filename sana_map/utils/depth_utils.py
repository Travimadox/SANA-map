...
def get_zed_camera_matrix():
    """Returns ZED camera parameters with alternative naming."""
    camera_matrix = {
        'fx': 1272.53,      # focal length x
        'fy': 1273.1,       # focal length y
        'f': 1272.815,       # focal length avg of fx and fy
        'xc': 980.676,      # principal point x (cx)
        'zc': 608.43,       # principal point y (cy)
        'k1': -0.0605594,   # radial distortion 1
        'k2': -0.0632102,   # radial distortion 2
        'k3': 0.00178986,   # radial distortion 3
        'p1': -0.000219042, # tangential distortion 1
        'p2': 0.000123048   # tangential distortion 2
    }
    camera_matrix = Namespace(**camera_matrix)
    return camera_matrix
...
def get_point_cloud_from_z_t_ros(Y_t, camera_matrix, device, scale=1):
    """Projects depth image into 3D point cloud with ROS coordinate system.
    Outputs:
        X is positive forward (depth direction)
        Y is positive left
        Z is positive up
        XYZ is ...xHxWx3
    """
    ...
    # Compute in CV coordinates first
    X_t = (grid_x[:, ::scale, ::scale] - camera_matrix.xc) * \
        Y_t[:, ::scale, ::scale] / camera_matrix.f
    Z_t = (grid_z[:, ::scale, ::scale] - camera_matrix.zc) * \
        Y_t[:, ::scale, ::scale] / camera_matrix.f
    ...
    X_ros = Y_t[:, ::scale, ::scale]  # Forward (depth)
    Y_ros = -X_t                      # Left (negative right)
    Z_ros = Z_t                        # Up (unchanged)

    XYZ = torch.stack((X_ros, Y_ros, Z_ros), dim=len(Y_t.size()))
