...
from ultralytics import YOLO,YOLOE, FastSAM, SAM
import torch
import os
...
class Yolo_E():
    def __init__(
        ...
        print("Initialising YoloE Detection & Segmentation")
    ...
    def get_predictions_with_sam(self, img, conf_threshold, iou_threshold):
        model_predictions = self.model(
            ...
            conf=conf_threshold,
            iou= iou_threshold,
            #imgsz=1920, # Specific to zed2i with full hd
            #imgsz=1280
        ...
        #print(f"{model_predictions[0].masks}")
        confidences = model_predictions[0].boxes.conf.cpu().numpy()
        class_ids = model_predictions[0].boxes.cls.cpu().numpy().astype(int)
        ...
        masks = model_predictions[0].masks.data.cpu().numpy()
        ...
        for cat, mask in zip(class_ids,masks):
            idx = int(cat)
            obj_mask = mask * 1.0
            if obj_mask.shape[:2] != (orig_h, orig_w):
                obj_mask = cv2.resize(obj_mask, (orig_w, orig_h), interpolation=cv2.INTER_NEAREST)
